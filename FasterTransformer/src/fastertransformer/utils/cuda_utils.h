...
template<typename T>
void printTensorFormat(const T* d_cache, int d0, int d1, int d2, int d3, char* name, char* format)
{
    int len = d0 * d1 * d2 * d3;
    ...
    printf("%s [%d][%d][%d][%d]", name, d0, d1, d2, d3);
    for (int i = 0; i < d0; i++) {
        for (int j = 0; j < d1; j++) {
            for (int k = 0; k < d2; k++) {
                printf("\n[%d][%d][%d]:\t", i, j, k);
                if(d3 > 15) {
                    for (int l = 0; l < 5; l++) {
                        printf(format, (float)res[cnt++]);
                    }
                    cnt += d3 - 10;
                    printf("\t...\t");
                    for (int l = d3 - 5; l < d3; l++) {
                        printf(format, (float)res[cnt++]);
                    }
                } 
                else    {
...
template<typename T>
void printTensor(const T* d_cache, int d0, int d1, int d2, int d3, char* name)
{
    printTensorFormat(d_cache, d0, d1, d2, d3, name, "%6.3f\t");
}

template<typename T>
void printTensorLong(const T* d_cache, int d0, int d1, int d2, int d3, char* name)
{
    printTensorFormat(d_cache, d0, d1, d2, d3, name, "%6.10f\t");
}

template<>
inline void printTensor<__half>(const __half* d_cache, int d0, int d1, int d2, int d3, char* name)
{
    int len = d0 * d1 * d2 * d3;
    __half* res = (__half*)malloc(sizeof(__half) * len);
    int cnt = 0;

    cudaMemcpy(res, d_cache, sizeof(__half) * len, cudaMemcpyDeviceToHost);
    printf("fp16 %s [%d][%d][%d][%d]", name, d0, d1, d2, d3);
    for (int i = 0; i < d0; i++) {
        for (int j = 0; j < d1; j++) {
            for (int k = 0; k < d2; k++) {
                printf("\n[%d][%d][%d]:\t", i, j, k);
