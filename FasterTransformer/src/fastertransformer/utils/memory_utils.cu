...
template<typename T>
__global__ void cudaD2DcpyState(T* dst, T* src, const int d_model, const int len)
{
    for(int i = 0; i < len; i++) {
        int offset = i * d_model + threadIdx.x;
        dst[offset] = src[offset];
    }
}
template<typename T>
void invokeCudaD2DcpyState(T* dst, T* src, const int d_model, const int len, cudaStream_t stream)
{
    cudaD2DcpyState<<<1, d_model, 0, stream>>>(dst, src, d_model, len);
}
template void invokeCudaD2DcpyState(float* dst, float* src, const int d_model, const int len, cudaStream_t stream);
template void invokeCudaD2DcpyState(half* dst, half* src, const int d_model, const int len, cudaStream_t stream);
}  // namespace fastertransformer
