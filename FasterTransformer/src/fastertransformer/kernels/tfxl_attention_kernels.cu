 ...
 */
#include "src/fastertransformer/kernels/tfxl_attention_kernels.h"
...
/********************** Kernels ************************/
template<typename T>
void __global__ prepareMatrixesQ(T* q_buf,
                                T* q_buf_bd,
                                const T* query_buf,
...
/*
    int k_off0 = cat_len * hidden_dim;  // cat_len*head_num*size_per_head
    int i_off1 = hidden_dim;          // head_num*size_per_head
    int k_o_off1 = cat_len * size_per_head;
    int off2 = size_per_head;
*/
template<typename T>
void __global__ prepareMatrixesK(T* k_buf,
                                const T* key_buf,
                                const int off0,
                                const int i_off1,
                                const int o_off1,
                                int off2)
{
    ...
    if (head_loc < i_off1) {
        ...
        int index = batch * off0 + cat * i_off1 + head_loc;
        int index_out = batch * off0 + head * o_off1 + cat * off2 + loc;
...
/*
    int bd_off0 = pos_len * hidden_dim;  // pos_len*head_num*size_per_head
    int i_off1 = hidden_dim;          // head_num*size_per_head
    int bd_o_off1 = pos_len * size_per_head;
    int off2 = size_per_head;
*/
template<typename T>
void __global__ prepareMatrixesK_BD(T* k_buf_bd,
                                const T* k_head_r,
                                const int off0,
                                const int i_off1,
                                const int o_off1,
                                int off2)
{
    int batch = blockIdx.y;
    int pos = blockIdx.x;
    int head_loc = threadIdx.x; // < hidden_dim
    ...
    if (head_loc < i_off1) {
        ...
        int index = pos * i_off1 + head_loc;  //(pos, head_loc)
        int index_out = batch * off0 + head * o_off1 + pos * off2 + loc;
...
template<>
void __global__ prepareMatrixesQ(__half* q_buf,
                                __half* q_buf_bd,
...
template<>
void __global__ prepareMatrixesK(__half* k_buf,
                                ...
                                int off2)
{
    ...
    if (head_loc < i_off1) {
        int head = head_loc / off2;
        int loc = head_loc % off2;
        int h2_index = (batch * off0 + cat * i_off1 + head_loc) >> 1;

        int h2_index_out = (batch * off0 + head * o_off1 + cat * off2 + loc) >> 1;
...
*/
template<>
void __global__ prepareMatrixesK_BD(__half* k_buf_bd,
                                ...
                                int off2)
{
    int batch = blockIdx.y;
    int pos = blockIdx.x;
    int head_loc = threadIdx.x * 2; // < hidden_dim
    ...
    if (head_loc < i_off1) {
        ...
        int index = (pos * i_off1 + head_loc)>>1;  //(pos, head_loc)
        int index_out = (batch * off0 + head * o_off1 + pos * off2 + loc)>>1;

        // bd
        tmp = ((half2*)k_head_r)[index];
        ((half2*)k_buf_bd)[index_out] = tmp;
    }
}
...
/*dim3 grid_shift(batch_size, head_num, seq_len);
  ...
  int off0=head_num*seq_len*pos_len;
  int off1=seq_len*pos_len; 
  [batch][head][seq][pos or cat]
*/
template<typename T>
void __global__ relShiftBd_seq1(T* outMatrix, const T* inputMatrix, const int off0, const int off1, const int out_off0, const int out_off1, const int seq_len, const int pos_len)
{
    int batch = blockIdx.x;  //[0,7]
    int head = blockIdx.y;   //[0,11]
    int row = blockIdx.z;    //[0,127]  //seq == 1
    int col = threadIdx.x;   //[0,255]  //pos
    int cat_len = pos_len;

    if (col >= 0) {
        int input_index = batch * off0 + head * off1 + row * pos_len + col;
        int output_index = batch * out_off0 + head * out_off1 + row * cat_len + col;
        T idata = inputMatrix[input_index];

        outMatrix[output_index] = idata;
    }
}
/*  int threads = 512;
    int pos_dim1 = threads / pos_len;
    int pos_dim2 = pos_len / pos_dim1;
...
template<typename T>
void __global__ relShiftBd(T* outMatrix, const T* inputMatrix, const int off0, const int off1, const int seq_len, const int pos_len)
{
    int batch = blockIdx.x;  //[0,7]
    int head = blockIdx.y;   //[0,11]
    int row = blockIdx.z;    //[0,127]  //seq
    int col = threadIdx.x;   //[0,255]  //pos
    int mem_len = pos_len - seq_len;

    int input_index = (batch * off0 + head * off1 + row * pos_len) * 2 + col;
    if (col >= pos_len || row != 0) {
        T idata = inputMatrix[input_index];
        // int tmp_index=row*(pos_len-1)+row+col-seq_len;
        int tmp_index = row * pos_len + col - seq_len;
        int out_row = tmp_index / (pos_len - 1);
        int out_col = tmp_index % (pos_len - 1);
        if (out_col < pos_len) {
            int out_index = batch * off0 + head * off1 + out_row * pos_len + out_col;
...
template<>
void __global__
relShiftBd(__half* outMatrix, const __half* inputMatrix, const int off0, const int off1, const int seq_len, const int pos_len)
{
    int batch = blockIdx.x;                           //[0,7]
    int head = blockIdx.y;                            //[0,11]
...
/*dim3 grid_score(batch_size,head_num,seq_len);
  ...
  int voff0=head_num*cat_len*size_per_head;
  int v_o_off1=cat_len*size_per_head;
                                      ...
                                      const int voff2)
{
    ...
    int head = blockIdx.y;
    int seq1 = blockIdx.z;
    int seq2 = threadIdx.x; // cat position

    int offset = batch * off0 + head * off1 + seq1 * cat_len;
    ...
    // softmax(attn_score+offset,cat_len, seq2);
    __shared__ float s_sum, s_max;
    float tmp = seq2 < cat_len ? score : large_value;
    float max_val = blockReduceMax<float>(tmp);
    if (seq2 == 0) {
        s_max = max_val;
    }
    __syncthreads();
    float qk_tmp = seq2 < cat_len ? __expf((float)(tmp - s_max)) : 0.0f;
    ...
    if (seq2 < cat_len) {
        ...
        // voff0 = head_num * cat_len * size_per_head;
        // voff2 = size_per_head;
        // v_i_off1 = head_num * size_per_head;
        // v_o_off1 = cat_len * size_per_head;
        // value_buf(v_head_h) (batch, cat_len, head_num_, size_per_head_)
        // to make value_buf_trans[batch][head_num_][cat_len][size_per_head_]
        offset = seq1;
        while (offset < voff2) {
            index = batch * voff0 + seq2 * v_i_off1 + head * voff2 + offset;
            out_index = batch * voff0 + head * v_o_off1 + seq2 * voff2 + offset;
...
template<typename T>
__global__ void
addBias_layerNorm(T* out, const T* input, const T* bias, const T* gamma, const T* beta, int m, int cat_len, int n, float epsilon)
{
    int tid = threadIdx.x;
    ...
    int id_bias = blockIdx.x * cat_len * n + tid;
    local_out += (float)(input[id]);
    local_out += (float)(__ldg(&bias[id_bias]));
                                  ...
                                  float epsilon)
{
    ...
    int id_bias = (blockIdx.x * cat_len * n + tid * 2) >> 1;
    local_out_fp2 = __half22float2(__hadd2(input_ptr[id], __ldg(&bias_ptr[id_bias])));
                           ...
                           cudaStream_t stream)
{
    ...
    dim3 gridQ(seq_len, batch_size);
    dim3 gridK(cat_len, batch_size);
    dim3 gridK_BD(pos_len, batch_size);
    dim3 block(next_pow2(hidden_dim) / numPerThread<T>());
    prepareMatrixesQ<<<gridQ, block, 0, stream>>>(q_buf,
                                                q_buf_bd,
                                                query_buf,
                                                attr_bias_Q_w,
                                                attr_bias_Q_r,
                                                ...
                                                o_off1,
                                                off2);
    prepareMatrixesK<<<gridK, block, 0, stream>>>(k_buf,
                                                key_buf,
                                                k_off0,
                                                i_off1,
                                                k_o_off1,
                                                off2);
    prepareMatrixesK_BD<<<gridK_BD, block, 0, stream>>>(k_buf_bd,
                                                k_head_r,
                                                bd_off0,
                                                i_off1,
                                                bd_o_off1,
...
template<typename T>
void blockRelShiftBd(dim3& grid, dim3& block, int batch_size, int head_num, int seq_len, int pos_len)
{
    grid.x = batch_size;
    grid.y = head_num;
    grid.z = seq_len;
...
template<typename T>
void invokeRelShiftBd(int batch_size, int head_num, int seq_len, int pos_len, T* qk_buf_bd_shift, T* qk_buf_bd, cudaStream_t stream)
{
    dim3 grid_shift;
    dim3 block_shift;
    blockRelShiftBd<T>(grid_shift, block_shift, batch_size, head_num, seq_len, pos_len);
    int cat_len = pos_len;

    int off0 = head_num * seq_len * pos_len;
    int off1 = seq_len * pos_len;
    int out_off0 = head_num * seq_len * cat_len;
    int out_off1 = seq_len * cat_len;

//    relShiftBd<<<grid_shift, block_shift, 0, stream>>>(qk_buf_bd_shift, qk_buf_bd, off0, off1, seq_len, pos_len);
    relShiftBd_seq1<<<grid_shift, block_shift, 0, stream>>>(qk_buf_bd_shift, qk_buf_bd, off0, off1, out_off0, out_off1, seq_len, pos_len);
}
...
void invokeCalAttnScore(int batch_size,
                        ...
                        cudaStream_t stream)
{
    int off0 = head_num * seq_len * cat_len;
    int off1 = seq_len * cat_len;
    ...
    dim3 grid_score(batch_size, head_num, seq_len);
    dim3 block_score(next_pow2(cat_len));
...
void invokeAddResidualLayerNorm(int batch_size,
                                ...
                                cudaStream_t stream)
{
//  TODO : after updating addBias_layerNorm, use 'dim3 block(hidden_dim / numPerThread<T>());' instead
...
template void invokeRelShiftBd<float>(
    int batch_size, int head_num, int seq_len, int pos_len, float* qk_buf_bd_shift, float* qk_buf_bd, cudaStream_t stream);

template void invokeRelShiftBd<half>(
    int batch_size, int head_num, int seq_len, int pos_len, half* qk_buf_bd_shift, half* qk_buf_bd, cudaStream_t stream);
