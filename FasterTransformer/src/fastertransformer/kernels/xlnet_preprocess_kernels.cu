...
template<typename T>
void __global__ relativePosition(T* attr_k_head_r, int hidden_dim, int cat_len)
{
                ...
                cudaStream_t stream)
{
    ...
    relativePosition<<<grid_rel_position, block_rel_position, 0, stream>>>(attr_k_head_r, hidden_dim, cat_len);
}
