...
echo ""
# Model
ckpt_file=../../../../train/saved_model
#ckpt_file=../../../../train/full_saved_model
...
if [ "$seq_len" -lt 3 ]
then
    echo "seq_len < 3 so set max_seq_len to 3. check this later."
    max_seq_len=3
else
    max_seq_len=${seq_len}
fi

#cm="python ./convertInput.py -t $data_name -b $max_batch_size -d $data_dir  -l $max_seq_len -s $spiece_model_file -o $data_file -u 0"
#echo ""
#echo COMMAND: $cm
#$cm
...
then
    ...
    cm="python ./runData.py -i $data_file -o $output_file -m $ckpt_file -b $max_batch_size -r $real_batch_size -l $seq_len -f $use_float16 -n $index -e $mem_len -d $dynamic_mem_len"
    echo ""
    echo COMMAND: $cm
    $cm
else
    ...
    echo COMMAND: $cm
    $cm
    cm="python ./runData.py -i $data_file -o $output_file -j $json_file -m $ckpt_file_16 -b $max_batch_size -r $real_batch_size -l $seq_len -f $use_float16 -n $index -e $mem_len -d $dynamic_mem_len"
    echo COMMAND: $cm
    $cm
fi

# Run FasterXLNet to verify its correctness
cm="../../../build/bin/tfxl_correctness_example ${max_batch_size} ${num_layers} ${seq_len} ${head_number} ${size_per_head} ${num_token} ${mem_len} ${d_model} ${d_inner} ${data_file} ${model_file} ${output_file} ${use_float16} "
echo ""
echo COMMAND: $cm
$cm
