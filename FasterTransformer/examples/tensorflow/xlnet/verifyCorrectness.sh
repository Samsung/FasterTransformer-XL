...
cm="python ./convertInput.py -t $data_name -b $batch_size -d $data_dir  -l $max_seq_len -s $spiece_model_file -o $data_file -u 0"
echo ""
echo COMMAND: $cm
#$cm
...
then
    ...
    cm="python ./runData.py -i $data_file -o $output_file -j $json_file -m $ckpt_file -b $batch_size -l $seq_len -f $use_float16 -n $index -e $mem_len"
    echo ""
    echo COMMAND: $cm
    $cm
else
    ...
    echo COMMAND: $cm
    $cm
    cm="python ./runData.py -i $data_file -o $output_file -j $json_file -m $ckpt_file_16 -b $batch_size -l $seq_len -f $use_float16 -n $index -e $mem_len"
    echo COMMAND: $cm
    $cm
fi

# Run FasterXLNet to verify its correctness
cm="../../../build/bin/xlnet_correctness_example ${batch_size} ${num_layers} ${seq_len} ${head_number} ${size_per_head} ${num_token} ${mem_len} ${data_file} ${model_file} ${output_file} ${use_float16} "
echo ""
echo COMMAND: $cm
$cm
